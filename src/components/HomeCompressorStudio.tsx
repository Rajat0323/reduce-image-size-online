...
const FORMAT_OPTIONS = [
  { label: "Auto (keep best)", value: "auto" },
  { label: "JPG", value: "image/jpeg" },
  { label: "PNG", value: "image/png" },
  { label: "WEBP", value: "image/webp" },
] as const;

const TARGET_PRESETS: Array<{ label: string; value: string }> = [
...
export default function HomeCompressorStudio() {
  ...
  useEffect(() => {
    ...
    if (queryTarget) {
      ...
      if (Number.isFinite(n) && n > 0) {
        const match = TARGET_PRESETS.find((p) => p.value === String(n));
        if (match) {
  ...
  return (
    <div className="home-studio" id="compressor" onPaste={onPaste}>
      ...
      >
        ...
        {!item ? (
          <div className="home-studio-empty">
            ...
            >
              <p className="home-studio-kicker">Free · Private · In your browser</p>
              <h2>Drop an image to compress</h2>
              <p className="home-studio-lead">
                Drag & drop, browse files, or paste from clipboard. Works on phone and desktop.
              </p>
              <div className="home-studio-actions">
                ...
                <span className="home-studio-hint">or Ctrl/Cmd + V to paste</span>
              </div>
          ...
          <div className="home-studio-workspace">
                ...
                <AnimatePresence mode="wait">
                  {item.result ? (
                    ...
                    <motion.div
                      ...
                      animate={{ opacity: 1 }}
                    >
                      <p>Choose format and target size, then compress.</p>
                    </motion.div>
                  )}
                </AnimatePresence>
              </article>
            ...
            <div className="home-studio-controls">
              ...
              <label className="home-studio-field">
                ...
                <select value={targetPreset} onChange={(e) => setTargetPreset(e.target.value)}>
              ...
              <button
                type="button"
                className="home-studio-text-btn advanced-toggle"
              ...
              {showAdvanced && (
                <div className="home-studio-advanced">
                  ...
                  <label className="home-studio-check">
                    ...
                    />
                    Remove metadata (EXIF stripped on re-export)
                  </label>
              ...
              {removeMetadata && (
                <p className="home-studio-note">
                  Re-exporting through canvas removes camera EXIF and GPS from the downloaded file.
                </p>
              )}
            </div>
          </div>
      ...
      {error && <p className="home-studio-error" role="alert">{error}</p>}
    </div>
  );
}
